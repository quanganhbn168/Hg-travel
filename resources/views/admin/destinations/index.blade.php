...
<x-admin.index-card title="Danh sách điểm đến" description="Quản lý địa danh, khu vực và điểm đến nổi bật." icon="bi-geo-alt" :create-url="route('admin.destinations.create')" create-label="Thêm điểm đến" resource="destination" :order-start="$destinations->firstItem() ?? 1">
    <x-slot:filters>
        <form action="{{ route('admin.destinations.index') }}" method="GET" class="row g-3 align-items-end">
            <div class="col-lg-4"><label class="form-label" for="destination-search">Từ khóa</label><input id="destination-search" name="search" value="{{ request('search') }}" class="form-control" placeholder="Tên hoặc slug điểm đến"></div>
            <div class="col-lg-2"><label class="form-label" for="destination-type">Loại</label><select id="destination-type" name="type" class="form-select"><option value="">Tất cả</option>@foreach($types as $value => $label)<option value="{{ $value }}" @selected(request('type') === $value)>{{ $label }}</option>@endforeach</select></div>
            <div class="col-lg-2"><label class="form-label" for="destination-market">Thị trường</label><select id="destination-market" name="market" class="form-select"><option value="">Tất cả</option>@foreach($markets as $value => $label)<option value="{{ $value }}" @selected(request('market') === $value)>{{ $label }}</option>@endforeach</select></div>
    ...
    <div class="table-responsive"><table class="table table-hover align-middle"><thead><tr><th data-select-column class="text-center" style="width:48px"><input type="checkbox" class="form-check-input" data-check-all aria-label="Chọn tất cả"></th><th>Tên điểm đến</th><th>Điểm đến cha</th><th>Phân loại</th><th class="text-center">Tour</th><th class="text-center">Nổi bật</th><th class="text-center">Trạng thái</th><th class="text-end">Thao tác</th></tr></thead><tbody>
        @forelse($destinations as $destination)
            <tr data-record-id="{{ $destination->id }}"><td data-select-column class="text-center">@if(!$destination->is_system)<input form="admin-bulk-destination-form" type="checkbox" name="ids[]" value="{{ $destination->id }}" class="form-check-input" data-check-item aria-label="Chọn {{ $destination->name }}">@endif</td><td>@if($destination->is_system)<span class="fw-semibold">{{ $destination->name }}</span><span class="badge text-bg-light border ms-1">Hệ thống</span>@else<a href="{{ route('admin.destinations.edit', $destination) }}" class="fw-semibold text-decoration-none">{{ $destination->name }}</a>@endif<small class="d-block text-body-secondary">{{ $destination->slug }} · {{ $destination->children_count }} node con</small></td><td>{{ $destination->parent?->name ?? '—' }}</td><td><span class="badge text-bg-light border">{{ $types[$destination->type] ?? $destination->type }}</span><small class="d-block text-body-secondary">{{ $markets[$destination->market] ?? $destination->market }}</small></td><td class="text-center"><span class="badge rounded-pill text-bg-primary">{{ $destination->tours_count }}</span></td><td class="text-center"><x-toggle model="destination" :id="$destination->id" field="is_featured" :checked="$destination->is_featured" :disabled="$destination->is_system" /></td><td class="text-center"><x-toggle model="destination" :id="$destination->id" field="is_active" :checked="$destination->is_active" :disabled="$destination->is_system" /></td><td class="text-end"><div class="btn-group btn-group-sm"><a href="{{ route('admin.destinations.edit', $destination) }}" class="btn btn-default" title="{{ $destination->is_system ? 'Chỉnh sửa nội dung' : 'Chỉnh sửa' }}"><i class="bi bi-pencil-square"></i></a>@if(!$destination->is_system)<button type="submit" form="delete-destination-{{ $destination->id }}" class="btn btn-default text-danger" title="Xóa"><i class="bi bi-trash"></i></button>@endif</div></td></tr>
        @empty
