...
@section('content')
    ...
    <section class="home-impact-section {{ $impactBackdropUrl ? 'has-impact-backdrop' : '' }}" aria-labelledby="impact-title" data-aos="fade-up" data-home-parallax>
        ...
        <div class="container home-impact-content"><div class="section-heading text-center mx-auto"><h2 id="impact-title" class="section-title">{{ $impactTitle }}</h2></div><div class="row justify-content-center g-4 home-impact-stats">@foreach ($impactStats as $stat)<div class="col-12 col-sm-4"><div class="impact-stat"><strong>{{ $stat['number'] }}</strong><span>{{ $stat['label'] }}</span></div></div>@endforeach</div></div>
    </section>
    ...
    <section class="section-space home-discovery-section" aria-labelledby="discovery-title" data-aos="fade-up">
        <div class="container"><div class="section-heading home-discovery-heading"><div><span class="section-eyebrow">Khám phá theo châu lục</span><h2 id="discovery-title" class="section-title">Điểm đến các châu lục</h2></div><div class="home-discovery-summary"><p class="section-description">Ưu tiên hành trình quốc tế, đồng thời giữ trọn các điểm đến trong nước để bạn dễ chọn theo từng khu vực.</p><a class="section-more" href="{{ route('tours.index') }}">Xem tất cả tour <i class="bi bi-arrow-up-right"></i></a></div></div>
            @php($destinationTabs = $destinationTabs ?: [['key' => 'all', 'label' => 'Tất cả']])
            <div class="home-tabs" role="tablist" aria-label="Điểm đến theo châu lục">@foreach ($destinationTabs as $tab)<button class="home-tab {{ $loop->first ? 'is-active' : '' }}" type="button" role="tab" aria-selected="{{ $loop->first ? 'true' : 'false' }}" data-destination-tab="{{ $tab['key'] }}">{{ $tab['label'] }}</button>@endforeach</div>
            <div class="home-destination-grid" data-destination-grid>@forelse ($featuredDestinations as $destination)<div class="home-destination-item destination-bento-item {{ $loop->first ? 'is-bento-featured' : '' }}" data-destination-item="{{ $destination['tab_key'] }}"><article class="destination-card"><a class="destination-card-image d-block" href="{{ $destination['url'] }}">@if ($destination['image_url'])<img src="{{ $destination['image_url'] }}" alt="{{ $destination['name'] }}" loading="lazy">@else<div class="home-placeholder-image"><i class="bi bi-image"></i></div>@endif</a><div class="destination-card-body"><span class="destination-card-kicker">{{ $destination['tab_label'] }}</span><h3 class="destination-card-title"><a href="{{ $destination['url'] }}">{{ $destination['name'] }}</a></h3><p class="destination-card-meta">{{ $destination['tour_count'] }} hành trình đang chờ bạn</p>@if ($destination['summary'])<p class="destination-card-summary">{{ $destination['summary'] }}</p>@endif</div></article></div>@empty<div class="home-empty-state"><i class="bi bi-geo-alt"></i><strong>Điểm đến đang được cập nhật</strong><span>Những địa danh đầu tiên sẽ xuất hiện tại đây ngay khi được thêm từ hệ thống quản trị.</span></div>@endforelse<div class="home-empty-state home-destination-tab-empty" data-destination-empty hidden><i class="bi bi-geo-alt"></i><strong>Chưa có điểm đến trong khu vực này</strong><span>Nội dung sẽ hiển thị tại đây khi được cập nhật từ hệ thống quản trị.</span></div></div>
        </div>
    </section>
